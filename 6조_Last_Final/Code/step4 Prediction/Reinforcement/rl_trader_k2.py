...
import argparse # argparse란 프로그램에 필요한 인자를 사용자 친화적인 명령행 인터페이스로 쉽게 작성하도록 돕는 라이브러리 이다. 즉, command 창에서 프로그램 내의 인자를 조절하게끔 도와준다
import re
import os # 파이썬을 이용해 파일을 복사하거나 디렉터리를 생성하고 특정 디렉터리 내의 파일 목록을 구하고자 할 때 os 모듈을 사용
import pickle # 원하는 데이터를 자료형의 변경없이 파일로 저장하여 그대로 로드할 수 있다

from sklearn.preprocessing import StandardScaler


# Let's use eworld, hite, samil adj close as k1
# Let's use hanEx, kolon, yeongwha adj close as k2
def get_data():
  ...
  df = pd.read_csv('hanEx_kolon_yeongwha.csv')
  return df.values
...
class MultiStockEnv:
  ...
  Action: categorical variable with 27 (3^3) possibilities
    ...
    - 2 = buy
  """
  def __init__(self, data, initial_investment=50000000):
    # data
